...
using JustDelivered.iOS.ConfirmationPageRenderer;
using System.Diagnostics;

[assembly: ExportRenderer(typeof(ConfirmationPage), typeof(ConfirmationPageRenderer))]
namespace JustDelivered.iOS.ConfirmationPageRenderer
{
	public class ConfirmationPageRenderer : PageRenderer
	{
...
#pragma warning restore CS0809 // Obsolete member overrides non-obsolete member
		...
		public void TextConfirmation(string title)
		{
			...
			actionSheet.Clicked += delegate (object a, UIButtonEventArgs b) {
				...
				{
					var phone = myPage.GetPhone();
					var name = myPage.GetName();
					Debug.WriteLine("PHONE: " + phone);
					Debug.WriteLine("NAME: " + name);
					var url = "sms:/" + "/" + "open?addresses=4084760001,4158329643," + myPage.GetPhone() + "&body=Hello%20" + GetFirstName() + "%20and%20Prashant,%20This%20is%20Just%20Delivered.%20We%20just%20delivered%20your%20package%21";
		...
		public void SendMMS(string url)
		{
            try
            {
				Debug.WriteLine("URL 2: " + url);
				var u = NSUrl.FromString(url);
				Debug.WriteLine("U: "  + u.AbsoluteString);
				UIApplication.SharedApplication.OpenUrl(NSUrl.FromString(url));
				//if (url != null) { UIApplication.SharedApplication.OpenUrl(NSUrl.FromString(url)); }
            }
            catch(Exception mms)
            {
				Debug.WriteLine("MESSAGE ERROR MMS:" + mms.Message);
            }
